...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = tifffile.imread(os.path.join(self.img_dir, img_id + self.img_ext))

        cliplimit = np.mean(image) + 3 * np.std(image)
        clahe = cv.createCLAHE(clipLimit=cliplimit, tileGridSize=(80, 80))
...
def parse_args():
    ...
    parser.add_argument('--input_w', default=768, type=int,
                        help='image width')
    parser.add_argument('--input_h', default=768, type=int,
                        help='image height')
    ...
    # dataset
    parser.add_argument('--dataset', default='20220112_groundtruth',
                        help='dataset name')
...
def main():
    ...
    if config['loss'] == 'BCEWithLogitsLoss':
        weight2 = [1.06537722, 0.94218266, 0.5665378, 2.42376917]
        criterion = nn.BCEWithLogitsLoss(weight=torch.Tensor(weight2)).cuda()  # WithLogits 就是先将输出结果经过sigmoid再交叉熵
    else:
    ...
    model = model.cuda()
    # ============
    pth_path=r"C:\Users\Mengxi\Box\Mengxi\notebook\NEO-conv\python\DL_for_Med\8-unet_for_cell_segmentation\unet++\unet++\models\20220112_groundtruth_NestedUNet_woDS\model-0.7144051235665012-2022_03_05_13_50_34.pth"
    ...
    img_ids_all = glob(
        os.path.join(r'C:\Users\Mengxi\Box\Data\20220112_groundtruth\GFP_original', '*' + config['img_ext']))
    ...
    def train_transform(mean,std):
        train_transform = A.Compose([
            ...
            #.Normalize(mean=mean, std=std, max_pixel_value=1),#mean=(0.03684), std=(0.01488), max_pixel_value=1
    ...
    val_transform = A.Compose([
        A.ToFloat(max_value=65535.0),
        A.RandomCrop(config['input_h'], config['input_w']),
        # .Normalize(mean=(0.03684), std=(0.01488), max_pixel_value=1.0),
        # A.Normalize(),
        #        A.FromFloat(max_value=13108.0)
    ])
    ...
    train_dataset = Dataset(
        ...
        mask_dir=os.path.join(inputdir, config['dataset'], 'groundtruth_png'),
